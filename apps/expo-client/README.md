...
Canonical path:
- `/root/autodl-tmp/sign2text.app/apps/expo-client`

Compatibility path:
- `/root/autodl-tmp/sign2text.app/Sign2text`
...
- The confirmed working SLRT Online inference path currently lives in `/root/autodl-tmp/SLRT/Online/CSLR` and runs on the GPU server, not on-device.
...
```bash
cd /root/autodl-tmp/sign2text.app/apps/expo-client
npm install
npx expo start
```
