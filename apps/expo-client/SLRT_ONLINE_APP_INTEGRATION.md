...
This guide explains how to connect the current app in `/root/autodl-tmp/sign2text.app/apps/expo-client` to the working SLRT Online inference pipeline in `/root/autodl-tmp/SLRT/Online` so the product can translate real-time sign language video to text.
...
cd /root/autodl-tmp/SLRT/Online/CSLR
...
1. Create service code under the canonical backend folder, for example:
  - `/root/autodl-tmp/sign2text.app/backend/sign2text-ml/app`
  - or under `/root/autodl-tmp/SLRT/Online/service` if you later decide to move it closer to SLRT
