...
class TranslationService: ObservableObject {
    ...
    func startTranslation() {
        ...
        DispatchQueue.main.async {
            ...
            self.currentModel = self.selectedPreset.title
    ...
    func setBackendURL(_ newValue: String) {
        ...
        UserDefaults.standard.set(normalized, forKey: selectedPreset.backendURLStorageKey)
    }

    func selectPreset(_ preset: RecognitionBackendPreset) {
        guard preset != selectedPreset else { return }

        let nextURL = Self.loadBackendURL(for: preset)
        DispatchQueue.main.async {
            self.selectedPreset = preset
            self.backendURL = nextURL
            self.currentModel = preset.title
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    self.currentModel = health.modelName
                    self.connectionStatus = health.status == "ok" ? "Backend reachable" : health.status
                    if health.datasetPreset != self.selectedPreset.rawValue {
                        self.lastErrorMessage = "Backend is running \(health.modelName) (\(health.datasetPreset)), but the app is set to \(self.selectedPreset.title). Switch backend or update the app preset."
                    } else {
                        self.lastErrorMessage = nil
                    }
                }
    ...
    private func ensureBackendSession() async {
        ...
        do {
            ...
            let createRequest = BackendSessionCreateRequest(metadata: [
                ...
                "platform": "ios",
                "app": "sign2text-app",
                "datasetPreset": selectedPreset.rawValue,
            ])
    ...
    private static func loadSelectedPreset() -> RecognitionBackendPreset {
        RecognitionBackendPreset(
            rawValue: UserDefaults.standard.string(forKey: selectedPresetStorageKey) ?? ""
        ) ?? .cslDaily
    }

    private static func loadBackendURL(for preset: RecognitionBackendPreset) -> String {
        if let saved = UserDefaults.standard.string(forKey: preset.backendURLStorageKey), !saved.isEmpty {
            return saved
        }
...
private struct BackendHealthResponse: Decodable {
    ...
    let translationModelLoaded: Bool
